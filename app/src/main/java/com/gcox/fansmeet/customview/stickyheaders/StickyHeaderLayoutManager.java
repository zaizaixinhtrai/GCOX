package com.gcox.fansmeet.customview.stickyheaders;
...
public class StickyHeaderLayoutManager extends RecyclerView.LayoutManager {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int adapterPosition = firstViewAdapterPosition; adapterPosition < state.getItemCount(); adapterPosition++) {
            ...
            if (itemViewType == SectioningAdapter.TYPE_HEADER) {
                ...
                //check previous pos is not excess item size
                final int ghostPosition = adapterPosition - 1;
                if(ghostPosition < 0 || ghostPosition >= state.getItemCount()) return;
                // we need to back up and get the header for this ghostHeader
                View headerView = recycler.getViewForPosition(ghostPosition);
        ...
        if (totalVendedHeight < innerHeight) {
            // note: we're passing null for RecyclerView.Status - this is "safe"
    ...
    int computeScrollVectorForPositionStick(int targetPosition) {
        updateFirstAdapterPosition();
    ...
    class SmoothScroller extends LinearSmoothScroller {
        ...
        public PointF computeScrollVectorForPosition(int targetPosition) {
            return new PointF(0, StickyHeaderLayoutManager.this.computeScrollVectorForPositionStick(targetPosition));
        }
