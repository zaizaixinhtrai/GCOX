...
public class RangeSeekBar<T extends Number> extends  android.support.v7.widget.AppCompatImageView {
    ...
    private final void trackTouchEvent(MotionEvent event) {
        ...
        int currentMinValue = (Integer) getSelectedMinValue();
        int currentMaxValue= (Integer) getSelectedMaxValue();
        if(xPrevious == 0){
            xPrevious = x;
            return;
        }
        float distance = x - xPrevious;

        // move the item left
        if (Thumb.MIN.equals(pressedThumb) && !mSingleThumb) {
             currentMinValue = (Integer) normalizedToValue(screenToNormalized(x));
            if(currentMaxValue - currentMinValue <= maxGap && currentMaxValue - currentMinValue>= minGap){
                setNormalizedMinValue(screenToNormalized(x));
            }else if(currentMaxValue - currentMinValue > maxGap || currentMaxValue - currentMinValue <minGap)
            {
                float leftPosition = normalizedToScreen(normalizedMinValue) + distance;
                float rightPosition = normalizedToScreen(normalizedMaxValue) + distance;
                double minPosition = screenToNormalized(leftPosition);
                double maxPosition = screenToNormalized(rightPosition);
                xPrevious = x;
                if(minPosition == 0  || maxPosition == 1){
                    Log.d(RangeSeekBar.class.getSimpleName(), "REturn 1");
                    return;
                }
                setNormalizedMinValue(minPosition);
                setNormalizedMaxValue(maxPosition);
            }
         // move item right
        }else if(Thumb.MAX.equals(pressedThumb)){
            currentMaxValue = (Integer) normalizedToValue(screenToNormalized(x));
            if(currentMaxValue - currentMinValue <= maxGap && currentMaxValue - currentMinValue>= minGap){
...
//        String maxLabel = getContext().getString(R.string.demo_max_label);
        String minLabel = getAbsoluteMinValue().toString();
        String maxLabel = String.valueOf(Integer.parseInt(getAbsoluteMaxValue().toString()) - 1);

        minLabel = String.valueOf(DateHelper.convertMilisecondsToHoursFormat(Long.parseLong(minLabel)));
        maxLabel = String.valueOf(DateHelper.convertMilisecondsToHoursFormat(Long.parseLong(maxLabel)));
        ...
        if (!selectedValuesAreDefault) {
            ...
            minText = DateHelper.convertMilisecondsToHoursFormat(Long.parseLong(minText));
            maxText = DateHelper.convertMilisecondsToHoursFormat(Long.parseLong(maxText));
...
//                        normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f,
                        normalizedToScreen(normalizedMinValue) - minTextWidth * 0.5f - thumbHalfWidth,
            ...
            canvas.drawText(maxText,
                    normalizedToScreen(normalizedMaxValue) - maxTextWidth * 0.5f + thumbHalfWidth,
                    mDistanceToTop + mTextSize,
                    paint);
        }
    ...
    private Thumb evalPressedThumb(float touchX) {
        ...
        boolean middlePressed = isInMiddleThumbs(touchX, normalizedMinValue, normalizedMaxValue);
    ...
    private boolean isInMiddleThumbs(float touchX, double normalizedMinValue, double normalizedMaxValue){

        boolean min = (touchX - normalizedToScreen(normalizedMinValue)) > 0 ? true : false;
        boolean max = (touchX - normalizedToScreen(normalizedMaxValue)) < 0 ? true : false;
        if(min && max){
            return true;
        }else {
            return false;
...
//    	double test = Math.max(0d, Math.min(1d, Math.min(value, normalizedMaxValue)));
//    	if(Integer.parseInt(normalizedToValue(test).toString()) <= 20){
//    		
//    	}
