...
class SlidingTabStrip extends LinearLayout {
    ...
    private static final int DEFAULT_SELECTED_INDICATOR_COLOR = 0xFFBA0000;
        ...
        for (int i = 0; i < getChildCount(); i++) {
            if (TextView.class.isInstance(getChildAt(i))) {
                if (selectedPosition == i) {
                    ((TextView) getChildAt(i)).setTextColor(getTabColorizer().getIndicatorColor(i));
                } else {
                    ((TextView) getChildAt(i)).setTextColor(ContextCompat.getColor(getContext(), R.color.grey_text_color));
                }
            }
        }
    }
